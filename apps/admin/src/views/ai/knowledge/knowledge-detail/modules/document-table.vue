...
watch(
  ...
  newData => {
    ...
    // 判定是否存在“待处理”或“处理中”的文档
    const hasPending = newData.some(item => {
      // 1. 任何正在处理中 (1) 的
      if (item.embeddingStatus === 1 || item.questionStatus === 1) return true;

      // 2. 特殊逻辑：对于 WORKFLOW_FILE，状态 0 也代表待分配/待处理，需要轮询直到变动
      if (props.processType === 'WORKFLOW_FILE' && item.embeddingStatus === 0) return true;

      // 3. 全局逻辑：如果状态为 0 且 meta 指明任务已在 PENDING/STARTED 状态，说明后端已接管但未进入最终态
      const hasActiveMeta = item.statusMeta?.state_time?.['1']?.['0'] || item.statusMeta?.state_time?.['1']?.['1'];
      if (item.embeddingStatus === 0 && hasActiveMeta) return true;
...
// 是否需要显示文件上传区域（仅限本地文件、QA对、工作流文件类型）
const showFileUpload = computed(() => {
  return (
    props.processType === 'GENERIC_FILE' || props.processType === 'QA_PAIR' || props.processType === 'WORKFLOW_FILE'
  );
});
...
async function handleAddDocument() {
  if (props.processType === 'WORKFLOW_FILE') {
    // 拦截逻辑：判断是否绑定了已发布的 fileProcess(useType=2) 应用
    if (!props.dataset?.workflowId) {
      showWorkflowBindingDialog('workflowAppRequired');
      return;
    }
    // 进一步校验应用状态
    try {
      const { data: app } = await fetchAppDetail(props.dataset.workflowId);
      if (app) {
        if (app.status !== '1') {
          showWorkflowBindingDialog('workflowAppUnpublished');
          return;
        }
        if (app.useType !== '2') {
          showWorkflowBindingDialog('workflowAppInvalidType');
          return;
        }
      } else {
        showWorkflowBindingDialog('workflowAppRequired');
  ...
  } else if (props.processType === 'GENERIC_FILE' || props.processType === 'QA_PAIR') {
    // 通用文件和QA对使用内联上传
    uploadAreaVisible.value = !uploadAreaVisible.value;
...
function showWorkflowBindingDialog(
  errorKey: 'workflowAppRequired' | 'workflowAppUnpublished' | 'workflowAppInvalidType'
) {
  const contentMap: Record<string, string> = {
    workflowAppRequired: $t('ai.knowledge_detail.document.workflowAppRequired'),
    workflowAppUnpublished: $t('ai.knowledge_detail.document.workflowAppUnpublished'),
    workflowAppInvalidType: $t('ai.knowledge_detail.document.workflowAppInvalidType')
  };

  const content = contentMap[errorKey] || contentMap.workflowAppRequired;

  dialog.warning({
    title: $t('common.tip'),
    content,
    positiveText: $t('ai.knowledge_detail.document.goToEdit'),
    onPositiveClick: () => {
      emit('editDataset');
    }
  });
...
function getAddButtonText() {
  const textMap: Record<string, string> = {
    ...
    WORKFLOW_FILE: $t('ai.knowledge_detail.document.uploadWorkflow')
  };
...
function getAddButtonIcon() {
  const iconMap: Record<string, string> = {
    ...
    WORKFLOW_FILE: 'mdi:file-cog-outline'
  };
